...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, user: { id: newUser.rows[0].id, name: newUser.rows[0].name, email: newUser.rows[0].email, role: "customer" } });
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
  }
...
app.post("/api/orders", async (req, res) => {
  ...
  try {
    ...
    const orderRes = await db.query(
      ...
      [userEmail, total, shipping.name, shipping.address, shipping.city, shipping.zip, shipping.country, shipping.phone]
    );
    const orderId = orderRes.rows[0].id;
    for (const item of items) {
      await db.query(
        `INSERT INTO order_items (order_id, product_name, price, quantity, image) VALUES ($1, $2, $3, $4, $5)`,
